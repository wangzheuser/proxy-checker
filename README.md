...
自托管 Python 服务支持真正的后端自动模式，不靠浏览器 `setInterval` 凑数。浏览器关闭后，服务仍然可以按计划拉取全部免费代理源，合并“我的仓库”里的代理，按设置的轮次、并发和检测模式批量检测，再把符合策略的结果同步到仓库 TXT / JSON 链接。
...
- Vercel / Serverless 不支持后台常驻自动模式，会明确显示“不支持后台自动模式”。
